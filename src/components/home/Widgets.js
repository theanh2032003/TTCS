  ...
  return (
    <div className='widgets'>
      <div className="widgets__input" onClick={handleInputClick}>
        ...
        <input placeholder="Search Twitter" type="text" value={userNameSearch} onChange={(e) => setUserNameSearch(e.target.value)} />
      </div>

      <div className="widgets__widgetContainer">
        <h2>Who to follow</h2>
        {userDtos.map( userDto => {
         return <WidgetsOption key={userDto.id} userId={userDto.id} name={userDto.fullname} textId = {userDto.textId} avatar={userDto.avatar}/>
        })
        }
